...
- **`autolaunch.rs`** — platform-specific startup registration: Windows registry + VBScript wrapper; Linux `.desktop` file; macOS LaunchAgent plist
...
- **Auto-launch** — Windows: VBScript wrapper in `%APPDATA%` + registry `Run` key (hides console window); Linux: `.desktop` in `~/.config/autostart`; macOS: LaunchAgent plist
- **Icon generation** — `build.rs` generates `assets/icon-active.png` and `assets/icon-inactive.png` at compile time (16×16 RGBA); `tray.rs` embeds them with `include_bytes!`
