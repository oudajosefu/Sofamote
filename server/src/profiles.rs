...
fn key_mod(k: KeyName, mods: &[Modifier]) -> ActionRecipe {
    ActionRecipe { key: Some(k), mods: mods.to_vec(), combo: None }
}

fn combo(keys: &[KeyName]) -> ActionRecipe {
    ActionRecipe { key: None, mods: vec![], combo: Some(keys.to_vec()) }
}
...
static GENERIC: LazyLock<HashMap<ActionName, ActionRecipe>> = LazyLock::new(|| {
    ...
    m.insert(ActionName::SeekBack30, combo(&[KeyName::Left, KeyName::Left, KeyName::Left]));
    m.insert(ActionName::SeekFwd30,  combo(&[KeyName::Right, KeyName::Right, KeyName::Right]));
...
static YOUTUBE: LazyLock<HashMap<ActionName, ActionRecipe>> = LazyLock::new(|| {
    ...
    m.insert(ActionName::SeekBack30,  key_mod(KeyName::Left,   &[Modifier::Shift]));
    m.insert(ActionName::SeekFwd30,   key_mod(KeyName::Right,  &[Modifier::Shift]));
    m.insert(ActionName::VolUp,       key(KeyName::Up));
    m.insert(ActionName::VolDown,     key(KeyName::Down));
    m.insert(ActionName::Mute,        key(KeyName::M));
    m.insert(ActionName::Fullscreen,  key(KeyName::F));
    m.insert(ActionName::Captions,    key(KeyName::C));
    m.insert(ActionName::NextEpisode, key_mod(KeyName::N, &[Modifier::Shift]));
    m.insert(ActionName::SpeedDown,   key_mod(KeyName::Comma,  &[Modifier::Shift]));
    m.insert(ActionName::SpeedUp,     key_mod(KeyName::Period, &[Modifier::Shift]));
    m
});
...
pub fn resolve_action(profile: Option<ProfileName>, action: ActionName) -> Option<&'static ActionRecipe> {
